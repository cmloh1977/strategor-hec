...
// ── Main Component ──
// ══════════════════════════════════════════════
export default function InnovationDirectionsPane({ lang, level }: InnovationDirectionsPaneProps) {
  const {
    portfolio,
  ...
  // Use JSON.stringify to avoid infinite loop from new array references
  const savedSelectionsKey = JSON.stringify(savedSelections || []);
  useEffect(() => {
    const selections = savedSelections || [];
    setLocalSelectedIds(new Set(selections.map((s) => s.id)));
    const map: Record<number, string> = {};
    selections.forEach((s) => { map[s.id] = s.justification; });
  ...
  const confirmedDirections: DirectionEntry[] = useMemo(() => {
    if (!confirmed || !savedSelections) return [];
  ...
  const handleConfirm = useCallback(() => {
    ...
    // Notify ChatPane to show a "go to Deep Dive" prompt
    const selectedNames = [...localSelectedIds]
      .map((id) => DIRECTIONS.find((d) => d.id === id)?.name)
      .filter(Boolean);
    window.dispatchEvent(
      new CustomEvent("innovation-confirmed", {
        detail: { directions: selectedNames },
      })
    );
  }, [confirmInnovationSelections, localSelectedIds]);
